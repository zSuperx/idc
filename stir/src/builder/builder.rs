...
#[derive(Debug, Clone)]
pub struct IRBuilder<I: InstructionTrait> {
    cursor: BBID,
    pub reg_count: usize,
...
#[derive(Debug, Clone)]
pub struct IRFunction<I: InstructionTrait> {
    pub entrypoint: BBID,
    blocks: HashMap<BBID, BasicBlock<I>>,
}

impl<I: InstructionTrait> Default for IRBuilder<I> {
    fn default() -> Self {
        Self::empty()
    }
}
...
impl<I: InstructionTrait> IRBuilder<I> {
    pub fn empty() -> Self {
        Self {
    ...
    pub fn with_state<O: InstructionTrait>(other: &IRBuilder<O>) -> Self {
        let mut s = Self {
            cursor: other.cursor,
    ...
    pub fn get_current_function_name(&self) -> &'static str {
        self.cursor.0
    }

    pub fn get_current_function(&mut self) -> &mut IRFunction<I> {
        self.functions.get_mut(self.cursor.0).unwrap()
    }
    ...
    pub fn newBlock(&mut self, function_name: &'static str) -> BBID {
    ...
    pub fn newNamedBlock(&mut self, function_name: &'static str, block_name: &'static str) -> BBID {
        let function = self.functions.get_mut(function_name).unwrap();
...
impl<I: InstructionTrait> IRFunction<I> {
