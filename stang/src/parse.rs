...
impl GlobalState {
    pub fn parse_file(&mut self, filename: &'static str) -> Vec<Spanned<HirObj>> {
        let source_code = std::fs::read(filename).unwrap();
        ...
        self.col = 0;
        let mut objects = vec![];
        while self.peek().inner != Token::Eof {
            objects.push(self.parse_obj())
        }
        objects
    }
    ...
    fn parse_func(&mut self) -> Spanned<HirObj> {
        ...
        let obj = HirObj::Fn(ParsedFunction {
            name,
            returns,
            args,
            body,
